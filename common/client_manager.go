...
func initClientConfig(fee string, host string, gas ...uint64) (cfg types.ClientConfig) {
	if fee == AUTO {
		cfg, _ = types.NewClientConfig(host, "okexchainevm-8", types.BroadcastBlock, "", 200000, 1.5, "0.00000001"+NativeToken)
	} else {
		if len(gas) != 0 {
			cfg, _ = types.NewClientConfig(host, "okexchainevm-8", types.BroadcastBlock, fee, gas[0], 0, "")
		} else {
			cfg, _ = types.NewClientConfig(host, "okexchainevm-8", types.BroadcastBlock, fee, 200000, 0, "")
		}
	}
	return
}
